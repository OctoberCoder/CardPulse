...
class ReloadlyClient:
    ...
    def __init__(self):
        ...
        self._is_sandbox = self.settings.reloadly_environment == "sandbox"
        if self._is_sandbox:
            self._api_base = "https://giftcards-sandbox.reloadly.com"
            self._audience = "https://giftcards-sandbox.reloadly.com"
        else:
            self._api_base = "https://giftcards.reloadly.com"
            self._audience = "https://giftcards.reloadly.com"
        self._token = None

    async def _get_token(self) -> str:
        async with httpx.AsyncClient() as client:
            resp = await client.post("https://auth.reloadly.com/oauth/token", json={
    ...
    async def list_products(self, page: int = 1, size: int = 50) -> list[dict]:
        result = await self._request("GET", f"/products?page={page}&size={size}")
    ...
    async def place_order(self, product_id: int, quantity: int, recipient_email: str) -> dict:
        result = await self._request("POST", "/orders", json={
    ...
    async def get_balance(self) -> dict:
        ...
        try:
            return await self._request("GET", "/accounts/balance")
        except Exception:
            return {"balance": 0.0, "note": "Balance endpoint unavailable in sandbox"}
